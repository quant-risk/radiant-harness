...
func pathIsSafe(projectDir, candidate string) bool {
	...
	// Resolve the candidate path by walking up until we find something that
	// exists. For "evil/target.txt" where evil is a symlink but target.txt
	// doesn't exist, we resolve "evil" and detect the escape.
...
// resolveLongestExistingPrefix walks up `full` until it finds a path that
// exists (file, dir, or symlink), then returns the EvalSymlinks result of
// that path. If no prefix exists, returns `full` unchanged.
