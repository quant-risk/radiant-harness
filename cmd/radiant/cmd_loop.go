...
func registerLoopCmds(root *cobra.Command) {
	...
	loopStartCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Light build: apiKey and baseURL are always empty (no HTTP LLM).
			// Inference is delegated to the host agent via MCP sampling — the
			// loop calls `llm.NewClient(m).Chat()` which routes through the
			// SamplingBackend. If no host agent is connected, Chat returns
			// ErrNoHostAgent with a clear "run `radiant setup-mcp` first" hint.
